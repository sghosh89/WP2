...
nrow(md_now)==1
if(md_now$STUDY_ID!=md_now$newsite){
  resloc_output<-paste(resloc,dataset_id,"/",md_now$newsite,"/",sep="")
  if(!dir.exists(resloc_output)){
    dir.create(resloc_output)
  }
}
newsite<-md_now$newsite
#----------------------------------------------------------
resloc_input<-paste("../DATA/for_BioTIMEx/wrangled_data/",dataset_id,"/",sep="")
input_tailanal<-readRDS(paste(resloc_input,dataset_id,"_",md_now$newsite,"_inputmatrix_tailanal.RDS",sep=""))
...
if(suffyr==T){
  ...
  tempdat<-env_BTx%>%filter(STUDY_ID%in%dataset_id)%>%filter(newsite%in%newsite)%>%filter(yr%in%rownames(input_tailanal))%>%dplyr::select(yr,t,tmax,tmin)
  ...
  saveRDS(input_tailanal,paste(resloc_input,"input_mat_for_tailanal_with_env_",dataset_id,"_",newsite,".RDS",sep="")) # dataframe with species timeseries along column
