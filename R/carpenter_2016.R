...
ddata <- ddata%>%filter(site=="Paul Lake")%>%dplyr::select(c(year,daynum,date,standard_taxon_code,group_code,species,value))
...
spmat<-ddata%>%dplyr::select(year, species=Species_agg, value)
