---
title: Preliminary report with traditional stability, Effect of climatic extremes on community stability and its drivers
...
if(!dir.exists("Results/res_Prelim_Report")){
  dir.create("Results/res_Prelim_Report")
}

if(!dir.exists("Results/res_Prelim_Report/traditional_stability_res/")){
  dir.create("Results/res_Prelim_Report/traditional_stability_res/")
}
```
...
gb_stab2<-ggplot(data=sb,aes(x=t_med_celcius, y=iCV, col = richness_cat)) +
  geom_point(alpha=0.2) +
  geom_smooth(method="lm", se=T) +
  ylab("Stability") +
...
bird_stab1 <- lm(iCV ~ nsp * t_med_celcius, data = sb)
par(mfrow = c(2,2))
#plot(bird_stab1)
...
gf_stab2<-ggplot(data=sfi,aes(x=t_med_celcius, y=iCV, col = richness_cat)) +
  geom_point(alpha=0.2) +
  geom_smooth(method="lm", se=T) +
  ylab("Stability") +
  xlab("Temperature, MedianT (\u00B0C)") +
...
fish_stab1 <- lm(log2(iCV) ~ log2(nsp) * t_med_celcius, data = sfi)
par(mfrow = c(2,2))
#plot(fish_stab1)
...
call_model_psem<-function(taxa,data=sm_all,
  ...
  vf<-car::vif(lme4::lmer(stability ~ R+E+VR+A+t.sens.slope.celcius+t_skw_celcius+t_var_celcius+(1|UID), data=mydat_scaled))
...
#----- call for birds -----
taxa<-"birds"
ans1<-call_model_psem(taxa = taxa, data=sm_all,
                rescaling = rescaling,  
                mean_centering = mean_centering,
                resloc=resloc)
saveRDS(ans1,here(paste(resloc,"model_psem_goodfit_",taxa,".RDS",sep="")))
#----- call for fish -----
taxa<-"fish"
ans1<-call_model_psem(taxa = taxa, data=sm_all,
                rescaling = rescaling,  
...
taxa<-"fish"
ans1<-call_model_psem(taxa = taxa, data=sfi_A_non0,
                rescaling = rescaling,  
                mean_centering = mean_centering,
                resloc=resloc)
dev.off()
...
```{r psem_fish_subset, echo=F}
print(summary(ans1))
```


```{r insert-pdf, out.height = "460px", out.width='800px'}
knitr::include_graphics(here("Results/res_Prelim_Report/traditional_stability_res/plot_psem_goodfit.pdf"),rel_path = F)
```

```{r print-model-summary, echo=FALSE, cache=T, cache.extra=list(mtime(here("Results/res_Prelim_Report/traditional_stability_res/model_psem_goodfit_birds.RDS")), mtime(here("Results/res_Prelim_Report/traditional_stability_res/model_psem_goodfit_fish.RDS")))}
...
```{r plot-stability-taildepsyn, echo=F}
#----- stability vs tail-dep synchrony raw plot for each taxa -----
library(ggpubr)
# with all data
sm_all<-read.csv(here("Results/stability_metric_and_env_all.csv"))
mydat<-sm_all
...
g0<-mydat %>%
  ...
  stat_cor(aes(label = paste(..r.label..,..rr.label.., ..p.label.., sep = "*`,`~")),
           label.x = -0.7, label.y = 20,col="black")+
  stat_regline_equation(label.x = -0.7, label.y = 18)
print(g0)
...
g0<-mydat %>%
  filter(TAXA%in%c("birds","fish")) %>%
  ggplot(aes(x=log10(A),y=log10(iCV))) +
  geom_point(alpha=0.3) +
  geom_smooth(method="lm") +
