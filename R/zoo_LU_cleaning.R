...
meta_swisszoo<-read.csv(paste(path,"zooplankton_MD.csv",sep=""),sep=";")
xm<-meta_swisszoo%>%dplyr::select(id_CH,empire,kingdom,phylum,
                class,order,family,genus,species,stage,
                affiliation,synonyms,comments,size_category)
#---------------------------------------------------------------------
# lake vierwaldstdttersee/ lake lucerne
...
l3z_lu_l_3A01<-inner_join(l3z_lu_l_3A01,splist_l3lu_A301_BM,by=c("genus_sp"))%>%
  dplyr::select(year,month,day,value,sp=aggregation)
...
saveRDS(commonspmat,
        paste(resloc_z,"input_mat_for_tailanal_zoo_LU.RDS",sep=""))
