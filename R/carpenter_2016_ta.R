...
if(nrow(md_now)==1){
  resloc_output<-paste(resloc,dataset_id,"/",sep="")
...
input_tailanal<-readRDS(paste(resloc_input,dataset_id,"_inputmatrix_tailanal.RDS",sep=""))
...
if(suffyr==T){
  ...
  saveRDS(input_tailanal,paste(resloc_input,"input_mat_for_tailanal_with_env.RDS",sep="")) # dataframe with species timeseries along column
