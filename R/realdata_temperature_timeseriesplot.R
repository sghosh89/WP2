...
tempo<-sbf%>%filter(t.lm.slope.sig==0 & t.sens.slope.sig.celcius==0)
eps=0.1e-2
id<-which(abs(tempo$t_skw_celcius-0)<eps)
...
tempo<-sbf%>%filter(t.lm.slope.sig==0 & t.sens.slope.sig.celcius==0)
tempo_c<-tempo[which.min(tempo$t_skw_celcius),]

# choose data: +ve trend, no sig. skewness
tempo<-sbf%>%filter(t.lm.slope.sig==1 & t.sens.slope.sig.celcius==1)
eps<-1e-3
tempo_d<-tempo[abs(tempo$t_skw_celcius-0)<eps,]
tempo_d<-tempo_d[2,]# choose the second entry

# choose data: +ve trend, with +ve skewness
tempo<-sbf%>%filter(t.lm.slope.sig==1 & t.sens.slope.sig.celcius==1)
id<-which.max(tempo$t_skw_celcius)
tempo_e<-tempo[id,]
