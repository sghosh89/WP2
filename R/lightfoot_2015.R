...
x<-x%>%dplyr::select(YEAR,SAMPLING_TIME,DATE,SITE,TRANSECT=TRN,SPECIES,COUNT=CNT,BURNED)
...
get_data_EarlyLate<-function(x,dataset_id='lightfoot_2015',resloc,samplingtime){
  ...
  # for site BOER
  ddata_BOER<-ddata%>%filter(SITE=="BOER")%>%dplyr::select(YEAR,SPECIES,COUNT)%>%
    spread(SPECIES,COUNT)
  ddata_BOER<-as.data.frame(ddata_BOER)
  rownames(ddata_BOER)<-ddata_BOER$YEAR
  ddata_BOER<-ddata_BOER%>%dplyr::select(-YEAR)
  ...
  # for site LATR
  ddata_LATR<-ddata%>%filter(SITE=="LATR")%>%dplyr::select(YEAR,SPECIES,COUNT)%>%
    spread(SPECIES,COUNT)
  ddata_LATR<-as.data.frame(ddata_LATR)
  rownames(ddata_LATR)<-ddata_LATR$YEAR
  ddata_LATR<-ddata_LATR%>%dplyr::select(-YEAR)
