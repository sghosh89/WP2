...
ggplot()+
  ...
  geom_smooth(data=sb,aes(x=nsp,y=iCValt, colour="black"),method="lm", se=F) +
  geom_smooth(data=lowT_taxa,aes(x=nsp,y=iCValt, colour="blue"),method="lm", se=F)+
  geom_smooth(data=highT_taxa,aes(x=nsp,y=iCValt, colour="red"),method="lm", se=F)+
  scale_x_continuous(trans = 'log2') +
...
ggplot()+
  ...
  geom_smooth(data=sb,aes(x=phi_LdM,y=iCValt, colour="black"),method="lm", se=F) +
  geom_smooth(data=lowT_taxa,aes(x=phi_LdM,y=iCValt, colour="blue"),method="lm", se=F)+
  geom_smooth(data=highT_taxa,aes(x=phi_LdM,y=iCValt, colour="red"),method="lm", se=F)+
  scale_x_continuous(trans = 'log2') +
  scale_y_continuous(trans = 'log2')+
...
From the above plot you can see that fish species are more or less equally divided showing positive or negative correlation across sites, whereas birds are more consistent in positive response towards warming across sites. So, imagine a scenario where warming kills some fish species across sites, then there would be qual chance of living fish that are varying in response. 
...
model_psem<-piecewiseSEM::psem(
  lmer(VR ~ t_med+trend_t_tau+(1|UID),data=dat),
  lmer(A ~ t_skw+(1|UID),data=dat),
