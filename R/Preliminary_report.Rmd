...
Let $N_{i,t,s}$ be the abundance/biomass of species $i$ at time $t$ at site $s$. Total abundance at time $t$ at site $s$ is $N_{t,s} = \sum_{i=1}^{s} N_{t,s,i}$. 

*Community stability* at site $s$ was estimated as the inverse of the coefficient of temporal variation in total community *biomass ?or? abundance*: $TempStab_s = 1 / CV(N_{t,s}) = mean(N_{t,s}) / sd(N_{t,s})$
...
```{r exploratory-plots1, echo=F, out.width = '75%',fig.cap=c("Stability-diversity relationship for birds and fish.")}
library(dplyr)
library(tidyverse)
...
```{r exploratory-plots2, echo=F, out.width = '75%',fig.cap=c("Stability-diversity relationship for birds at different temperature levels")}
## stability versus richness, by temperature category
...
```{r exploratory-plots3, echo=F, out.width = '75%',fig.cap=c("Stability-temperature relationship for bird communities at different richness levels")}
## stability versus temperature, by richness category
...
```{r exploratory-plots4, echo=F, out.width = '75%',fig.cap=c("Stability-synchrony relationship for birds at different temperature levels")}
## stability versus synchrony, by temperature category
...
```{r exploratory-plots5, echo=F, out.width = '75%',fig.cap=c("Synchrony-temperature relationship (scatterplot)")}
## synchrony versus temperature
...
```{r exploratory-plots6, echo=F, out.width = '75%',fig.cap=c("Synchrony-temperature relationship (boxplot)")}
...
```{r exploratory-plots7, echo=F, out.width = '75%',fig.cap=c("Synchrony richness relationship.")}
...
```{r exploratory-plots8, echo=F, out.width = '75%',fig.cap=c("Synchrony temperature relationship.")}
## synchrony versus temperature, by richness category
...
```{r exploratory-plots8.1, echo=F, out.width = '75%',fig.cap=c("Stability - temperature skew relationship.")}
...
There is some suggestion that this may be explained by synchrony, but the statistics show now strong associations of synchrony with t_med or synchrony.
...
```{r exploratory-plots9, echo=F, out.width = '75%',fig.cap=c("Stability-diversity relationship at different temperature levels")}
## stability versus richness, by temperature category
ggplot(data=sb,aes(x=nsp,y=iCValt, col = median_temperature_cat)) +
  geom_point(alpha=0.2) +
  geom_smooth(method="lm", se=T) +
...
```{r exploratory-plots10, echo=F, out.width = '75%',fig.cap=c("Stability-temperature relationship at different richness levels")}
## stability versus temperature, by richness category
ggplot(data=sb,aes(x=t_med, y=iCValt, col = richness_cat)) +
  geom_point(alpha=0.2) +
  geom_smooth(method="lm", se=T) +
...
```{r exploratory-plots11, echo=F, out.width = '75%',fig.cap=c("Stability-synchrony relationship at different temperature levels")}
## stability versus synchrony, by temperature category
ggplot(data=sb, aes(x=phi_LdM, y=iCValt, col = median_temperature_cat)) +
  geom_point(alpha = 0.2) +
  geom_smooth(method="lm", se=T) +
...
```{r exploratory-plots12, echo=F, out.width = '75%',fig.cap=c("Synchrony-temperature relationship (scatterplot)")}
## synchrony versus temperature
ggplot(data=sb, aes(x=t_med, y=phi_LdM, col = median_temperature_cat)) +
  geom_point(alpha = 0.2) +
  geom_smooth(method="lm", se=T) +
...
```{r exploratory-plots13, echo=F, out.width = '75%',fig.cap=c("Synchrony-temperature relationship (boxplot)")}
#----- do a boxplot for synchrony at low/high temp, birds -----
ggplot(data=sb, aes(x=median_temperature_cat, y=phi_LdM)) +
  geom_boxplot(alpha = 0.2) +
  scale_y_continuous(trans = 'log2')+
...
```{r exploratory-plots14, echo=F, out.width = '75%',fig.cap=c("Synchrony richness relationship.")}
## synchrony versus richness, by temperature category
ggplot(data=sb,aes(x=nsp,y=phi_LdM, col = median_temperature_cat)) +
  geom_point(alpha=0.2) +
  geom_smooth(method="lm", se=T) +
...
```{r exploratory-plots15, echo=F, out.width = '75%',fig.cap=c("Synchrony temperature relationship.")}
## synchrony versus temperature, by richness category
ggplot(data=sb,aes(x=t_med, y=phi_LdM, col = richness_cat)) +
  geom_point(alpha=0.2) +
  geom_smooth(method="lm", se=T) +
...
```{r exploratory-plots15.1, echo=F, out.width = '75%',fig.cap=c("Stability - temperature skew relationship.")}
...
fish_stab1 <- lm(log2(iCValt) ~ log2(nsp) * t_med, data = sb)
par(mfrow = c(2,2))
#plot(fish_stab1)
...
```{r}
## bird stability model
fish_stab2 <- lm(log2(phi_LdM) ~ log2(nsp) * t_med, data = sb)
par(mfrow = c(2,2))
#plot(fish_stab2)
...
```{r model_psem, echo=F, cache=F, message=F, cache.extra=list(mtime(here("Results/stability_metric_and_env_all.csv")), mtime(here("R/plot_psem.R")))}
library(tidyverse)
library(lme4)
