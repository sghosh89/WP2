...
*Comunity variance ratio*: a measure of synchrony, scaled between 0 to 1 ([Loreau & Mazancourt](https://www.journals.uchicago.edu/doi/10.1086/589746)).
...
*Temperature trend*: Monotonic trend of annual temperature timeseries (computed by Mann-Kendall test).
...
```{r trend-density-plot,out.width = '75%', fig.cap=c("Distribution of temperature trend estimated by non-parametric Sen's slope, birds and fish separate. Fish communities generally experience more increasing trend.")}
# extra plots of trend density across taxa

sm_all_bf%>%
  ggplot( aes(x=t.sens.slope,y=..scaled..))+geom_density()+ylim(c(0, 1))+
  annotate("rect", xmin=min(sm_all_bf$t.sens.slope), xmax=0, ymin=0, ymax=1, alpha=0.15, fill="blue")+
  annotate("rect", xmin=0, xmax=max(sm_all_bf$t.sens.slope), ymin=0, ymax=1, alpha=0.15, fill="red")+
  theme_classic()+facet_wrap(~TAXA)
```
...
model_psem<-piecewiseSEM::psem(
  ...
  lmer(R~t_med+t.sens.slope+t_skw+(1|UID),data=dat),
  lmer(stability ~ (R + VR +A)+t_med+t.sens.slope+t_skw+
         (1|UID), data=dat))
cc<-coefs(model_psem)
...
**Why I did not choose t_var (variability in temperature timeseries) in the path model?**
...
[I separately ran models replacing t_med, and t_skw with t_var, for birds nothing changes, for fish I found a positive significant effect of t_var on stability - though I don't know why? t_var was not related to richness though. Is it something related to more environmental variability would foster less evenness in fish community? I know a plant community study in PNAS which uses evenness as a driver for stability as well as richness].

```{r model_psem_t_var, echo=F, cache=T, message=F, cache.extra=list(mtime(here("Results/stability_metric_and_env_all.csv")), mtime(here("R/plot_psem.R")))}

taxa<-"birds"
dat<-mydat_scaled%>%filter(TAXA==taxa)
n<-nrow(dat)
model_psem<-piecewiseSEM::psem(
  lmer(VR ~ R+t_var+trend_t_tau+(1|UID),data=dat),
  lmer(A ~ R+t_var+(1|UID),data=dat),
  lmer(R~t_var+trend_t_tau+(1|UID),data=dat),
  lmer(stability ~ (R + VR +A)+t_var+trend_t_tau+
         (1|UID), data=dat))
cc<-coefs(model_psem)
plot_psem(n,taxa,cc,layout="circle")
