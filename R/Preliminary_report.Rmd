...
plot_temp_on_map<-function(sm_all,t,taxa,scaleup){
  dat<-sm_all%>%filter(TAXA==taxa)%>%dplyr::select(STUDY_ID,newsite,nsp,LATITUDE,LONGITUDE,t_med,t_skw)
...
gex<-ggplot(mydat,aes(x=cortrait_var,y=phi_LdM))+geom_point(alpha=0.2)+
  ...
  xlab("community-wide correlation traits variability (absolute), fish, log10 scale")+ylab("VR, fish")+
  theme_bw()
print(gex)
#---- for birds -----
taxa<-"birds"
...
gexb<-ggplot(mydat,aes(x=cortrait_var,y=phi_LdM))+geom_point(alpha=0.2)+
  ...
  scale_x_continuous(trans = 'log10') +
  #scale_y_continuous(trans = 'log10')+
  xlab("community-wide correlation traits variability (absolute), birds, log10 scale")+ylab("VR, birds")+
  theme_bw()
print(gexb)
```
...
call_model_psem<-function(taxa,data=sm_all,
  ...
  vf<-car::vif(lmer(stability ~ R+VR+A+t.sens.slope+t_skw+t_var+(1|UID), data=mydat_scaled))
  print(vf)
  vf<-car::vif(lmer(stability ~ R+VR+A+t.sens.slope.celcius+t_skw_celcius+t_var_celcius+(1|UID), data=mydat_scaled))
  print(vf)
  vf<-car::vif(lmer(stability ~ R+E+VR+A+t.sens.slope+t_skw+t_var+(1|UID), data=mydat_scaled))
  print(vf)
  ...
  # model without evenness (both K/10, Celcius scale)
  ...
  model_psem_c<-piecewiseSEM::psem(
    lmer(VR ~ R+t_med_celcius+t.sens.slope.celcius+(1|UID),data=dat),
    lmer(A ~ R+t.sens.slope.celcius+t_skw_celcius+(1|UID),data=dat),
    lmer(R~t_med_celcius+t_var_celcius+t.sens.slope.celcius+(1|UID),data=dat),
    lmer(stability ~ (R*t_med_celcius)+VR+A+t.sens.slope.celcius+
           t_skw_celcius+t_var_celcius+(1|UID), data=dat))
  
  # model with evenness (both K/10, Celcius scale)
  model_psem2<-piecewiseSEM::psem(
    lmer(VR ~ R+E+t_med+t.sens.slope+(1|UID),data=dat),
    lmer(A ~ R+E+t.sens.slope+t_skw+(1|UID),data=dat),
    lmer(R~t_med+t_var+t.sens.slope+(1|UID),data=dat),
    lmer(E~R+t_med+t.sens.slope+t_var+(1|UID),data=dat),
    lmer(stability ~ R+E+VR+A+(R:t_med)+
           t_med+t_var+t.sens.slope+t_skw+(1|UID), data=dat))
 
  model_psem_c2<-piecewiseSEM::psem(
    lmer(VR ~ R+E+t_med_celcius+t.sens.slope.celcius+(1|UID),data=dat),
    lmer(A ~ R+E+t.sens.slope.celcius+t_skw_celcius+(1|UID),data=dat),
    lmer(R~t_med_celcius+t_var_celcius+t.sens.slope.celcius+(1|UID),data=dat),
    lmer(E~R+t_med_celcius+t.sens.slope.celcius+t_var_celcius+(1|UID),data=dat),
    lmer(stability ~ R+E+VR+A+(R:t_med_celcius)+
           t_med_celcius+t_var_celcius+t.sens.slope.celcius+
           t_skw_celcius+(1|UID), data=dat))
  
  op<-par(mfrow = c(2,2))
  
  # model without evenness (both K/10, Celcius scale)
  cc<-coefs(model_psem,standardize = "scale")
  ...
  # model with evenness (both K/10, Celcius scale)
  ccE<-coefs(model_psem2,standardize = "scale")
  plot_psem(n,taxa,ccE,layout="circle")
  saveRDS(model_psem2,paste(resloc,"model_psem_with_evenness_",taxa,".RDS",sep=""))
...
fish_meta<-fish_meta%>%dplyr::select(SpecCode,maxTtol,minTtol)
...
g1<-ggplot(mydat,aes(x=t_skw,y=avg_minTtol))+geom_point(alpha=0.2)+
  geom_smooth(method="lm",color="blue",fill="cornflowerblue")+
  #xlab("t_skw, K/10 unit")+
  #ylab("community-wide min tolerance limit \n
  #    variability (absolute), fish")+
  theme_bw()

g2<-ggplot(mydat,aes(x=t_skw,y=avg_maxTtol))+geom_point(alpha=0.2)+
  geom_smooth(method="lm",color="blue",fill="cornflowerblue")+
  #xlab("t_skw, K/10 unit")+
  #ylab("community-wide min tolerance limit \n
  #    variability (absolute), fish")+
