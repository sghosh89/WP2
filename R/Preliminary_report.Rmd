...
call_model_psem<-function(taxa,data=sm_all,
  ...
  dat<-rename(dat, MedianT = t_med_celcius, 
              ...
              SkewT = t_skw_celcius,
              VarT = t_var_celcius)
  # model without evenness (Celcius scale)
  #model_psem_c<-piecewiseSEM::psem(
  #  lmer(VR ~ R+MedianT+TrendT+(1|UID),data=dat),
  #  lmer(A ~ R+TrendT+SkewT+(1|UID),data=dat),
  #  lmer(R~MedianT+VarT+TrendT+(1|UID),data=dat),
  #  lmer(stability ~ (R*MedianT)+VR+A+TrendT+SkewT+VarT+(1|UID), data=dat))
  ...
  # model without evenness (Celcius scale)
  #cc2<-coefs(model_psem_c,standardize = "scale")
  #plot_psem(n,taxa,cc2,layout="circle")
  #saveRDS(model_psem_c,paste(resloc,"model_psem_celcius_",taxa,".RDS",sep=""))
  
  # model with evenness (Celcius scale)
  ccE2<-coefs(model_psem_c2,standardize = "scale")
  
  #plot_psem(n,taxa,ccE2,layout="circle")
  #pdf(here(paste(resloc,"plot_psem_",taxa,".pdf",sep="")),height=6,width=6)
  plot_psem(n,taxa,ccE2,layout="circle")
  #dev.off()
...
pdf(here(paste(resloc,"plot_psem.pdf",sep="")),height=12,width=6)
op<-par(mfrow=c(2,1))
...
knitr::include_graphics(here("Results/res_Prelim_Report/plot_psem.pdf"),rel_path = F)
```

```{r print-model-summary, echo=FALSE, cache=T, cache.extra=list(mtime(here("Results/res_Prelim_Report/model_psem_birds.RDS")), mtime(here("Results/res_Prelim_Report/model_psem_fish.RDS")))}
