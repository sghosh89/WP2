...
call_model_psem<-function(taxa,data=sm_all,
  ...
  vf<-car::vif(lmer(stability ~ R+VR+A+t.sens.slope.celcius+t_skw_celcius+t_var_celcius+(1|UID), data=mydat_scaled))
  print(vf)
  ...
  saveRDS(model_psem_c2,paste(resloc,"model_psem_with_evenness_celcius_",taxa,".RDS",sep=""))
...
```{r fish-skw-and-taildepsyn, echo=F}
library(here)
library(dplyr)
library(tidyverse)
library(gridExtra)
...
```{r birds-skw-and-taildepsyn, echo=F}
library(here)
library(dplyr)
library(tidyverse)
library(gridExtra)
