...
My intuition is:
  ...
  - Changing $t_{trend}$, $t_{skw}$, means frequent extreme events (like heatwaves, or cold winter), and it should be related to tail-dependent synchrony, where pairwise synchrony between species gets stronger beyond a certain threshold.
...
```{r trend-plot,out.width = '75%', fig.cap=c("Distribution of temperature trend estimated by non-parametric Sen's slope, and parametric linear fit slope. Colored points are significant Sen's slope (green: birds, blue: fish) .", "Histogram plot for trends, both taxa.","Histogram plot for significant trends, both taxa.")}
...
- for birds: increasing temperature $t_{med}$ decreases community stability, though it has no significant direct effect on the drivers (like richness, synchrony).
- increasing temperature trend also has no significant effect on birds.
- increasing extreme heatwaves decreases tail dep synchrony. Here direct effect of t_skw is not significant on stability, as all bird data have equal proportion of +ve and -ve skewness (see above plot). But still tail dependent synchrony is important mechanism for stability, especially for terrestrial taxa (we found this from BioDyn project), and driven by the skewness of temperature time series (extreme events).
...
- for fish, extremes has direct negative effect on stability, not indirect effect through tail-dep synchrony as it is not important driver for freshwater as we found from BioDyn.
...
model_psem<-piecewiseSEM::psem(
  ...
  lmer(A ~ R+t_var+t.sens.slope+t_skw+(1|UID),data=dat),
  lmer(R~t_var+t.sens.slope+(1|UID),data=dat),
  lmer(stability ~ (R + VR +A)+t_var+t.sens.slope+t_skw+
         (1|UID), data=dat))
cc<-coefs(model_psem)
plot_psem(n,taxa,cc,layout="circle")
