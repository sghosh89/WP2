...
r_BT_freshw$trend_t_tau<-NA # tau of Mann-Kendall trend test, for shorter time series it is difficult to see a trend
r_BT_freshw$trend_t_tau_sig<-NA # is the trend significant?
...
for(i in 1:nrow(r_BT_freshw)){
  ...
  r_BT_freshw$trend_t_tau<-unname(trend_mk$estimates["tau"]) # tau of Mann-Kendall trend test, for shorter time series it is difficult to see a trend
  r_BT_freshw$trend_t_tau_sig<-ifelse(trend_mk$p.value<0.05,1,0) # 1 means significant trend
